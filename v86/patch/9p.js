...
Virtio9p.prototype.ReceiveRequest = async function (bufchain) {
    ...
    switch(id)
    {
        ...
        case 12: // tlopen
            ...
            var ret = await this.fs.OpenInodeAsync(idx, mode);
        ...
        case 116: // read
            ...
            // if(id === 116) console.log("[read]: fid=" + fid + " (" + this.fids[fid].dbg_name + ") offset=" + offset + " count=" + count + " fidtype=" + this.fids[fid].type);
            if(!inode || inode.status === STATUS_UNLINKED)
            {
            ...
            if(this.fids[fid].type === FID_XATTR) {
                ...
                await this.fs.OpenInodeAsync(this.fids[fid].inodeid, undefined);
                ...
                // console.log("pre-read", count);
                const data = await this.fs.Read(inodeid, offset, count);
                
                this.bus.send("9p-read-end", [this.fids[fid].dbg_name, count]);

                // progrium: set count to data length if data is not null.
                // linux seems to keep trying if the file size is larger than whats been read.
                // not sure if we're return EOF correctly or if that's just what it does.
                if(data) {
                    // console.log("post-read", data.length);
                    count = data.length;
        ...
        case 110: // walk
            ...
            var nwfid = req[1];
            var nwname = req[2];
            // console.log("[walk]: fid=" + req[0] + " nwfid=" + req[1] + " nwname=" + nwname);
            ...
            for(var i=0; i<nwname; i++) {
                idx = await this.fs.SearchAsync(idx, walk[i]);
